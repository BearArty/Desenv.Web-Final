...
const { User, Jogo, Jogando } = require('../models');
...
exports.postCadastrar = async (req, res) => {
    const { nome, nickname, email, senha, genero_fav } = req.body;

    try {
        const senhaHash = bcrypt.hashSync(senha, 10);
        await User.create({nome, nickname, email, senha: senhaHash, genero_fav});
        // res.redirect("/login");
...
// Exibir perfil do usuário com os jogos e status de leitura
exports.getPerfil = async (req, res) => {
    ...
    try {
      ...
      const gaming = await Jogando.findAll({ where: { id_usuario: userId } });
      const jogandoMap = {};
      
      gaming.forEach(jogando => {
        jogandoMap[jogando.id_jogo] = jogando.status_jogo;
      });
  
      // Renderizar a página do perfil com os livros e o status de leitura
      // Passar o mapa de leituras do usuário
      res.render('perfil', { userName: req.session.userName, jogos, jogandoMap });
  
    } catch (error) {
      console.error(error);
      res.render('perfil', { userName : req.session.userName, jogos: [], error: 'Error ao carregar os jogos' });
    }
};
...
exports.postEditPerfil = async (req, res) => {
    ...
    try {
      ...
      await User.update({ nome, nickname, email, senha: hashedPassword, genero_fav }, { where: { id_usuario: userId } });
...
exports.postEditUser = async (req, res) => {
    ...
    try {
        const hashedPassword = bcrypt.hashSync(senha, 10);
        await User.update({ nome, nickname, email, senha: hashedPassword, genero_fav }, { where: { id_usuario: req.params.id } });
